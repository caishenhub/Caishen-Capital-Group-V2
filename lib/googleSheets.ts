...
export const setSecuritySession = (userId: string | null, isAdmin: boolean) => {
  const finalAdmin = isAdmin || String(userId || '').toUpperCase().startsWith('#ADM');
  currentSession = { userId, isAdmin: finalAdmin };
};
...
async function fetchFromServer(tabName: string): Promise<any[]> {
  ...
  inFlightRequests[tabName] = (async () => {
    console.log(`[ENGINE v4.5] Sincronizando ${tabName}...`);
    try {
      const params = new URLSearchParams({
        tab: tabName,
        token: GOOGLE_CONFIG.SECURITY_TOKEN
      ...
      const response = await fetch(`${PROFILE_API_URL}?${params.toString()}`);
      ...
      if (Array.isArray(data)) {
        ...
        // Filtrado por seguridad si es SOCIO (Scoping en cliente ya que el script devuelve todo)
        const sensitiveTabs = ['LIBRO_ACCIONISTAS', 'DATOS_PAGO_SOCIOS', 'DIVIDENDOS_SOCIOS', 'REPORTES_SOCIOS'];
        if (sensitiveTabs.includes(tabName) && !currentSession.isAdmin && currentSession.userId) {
          const scopedData = processedData.filter((row: any) => {
            const rowUid = String(findValue(row, ['UID_SOCIO', 'uid']) || '').toLowerCase();
            return rowUid === currentSession.userId?.toLowerCase();
          });
          setLocalCache(tabName, scopedData);
          return scopedData;
        }
      ...
      if (data && data.error) {
        console.warn(`Error script en ${tabName}:`, data.error);
      }
      
      return localCached?.data || [];
    } catch (err) {
      console.error(`Fallo crítico en ${tabName}:`, err);
      return localCached?.data || [];
    }
  })();
...
export async function sendToScript(payload: any) {
  try {
    ...
    try {
      ...
      console.warn("Could not parse script response as JSON:", text);
      return { 
    ...
    if (err.name === 'AbortError') {
      console.warn("Script request timed out");
      return { success: false, error: "TIMEOUT" };
    }
    console.warn("Communication error (POST):", err.message || err);
...
export async function publishNotification(notice: Partial<CorporateNotification>): Promise<{success: boolean; error?: string}> {
  ...
  return { success: !!(res && (res.success === true || !res.error)), error: res?.error };
}

export async function updateShareholderPin(uid: string, newPin: string): Promise<{success: boolean; error?: string}> {
  const res = await sendToScript({ 
    action: 'UPDATE_PIN', 
    uid: uid, 
    newPin: newPin
...
export async function updateShareholderProfile(uid: string, updates: any): Promise<{success: boolean; error?: string}> {
  const res = await sendToScript({ 
...
export async function fetchUserByEmailOrId(identifier: string): Promise<any | null> {
  // Con el script v4.5, obtenemos la tabla y buscamos localmente
  const users = await fetchTableData('LIBRO_ACCIONISTAS');
  const idNorm = norm(identifier);
  
  return users.find(u => {
    const uid = norm(findValue(u, ['UID_SOCIO', 'uid']));
    const email = norm(findValue(u, ['EMAIL_SOCIO', 'email']));
    return uid === idNorm || email === idNorm;
  }) || null;
}

export async function saveShareholderAccount(uid: string, accountData: any): Promise<{success: boolean; error?: string}> {
  // Con el script v4.5 no hay SAVE_ACCOUNT especializado, usamos append en DATOS_PAGO_SOCIOS
  const res = await sendToScript({
    action: 'append',
  ...
  const success = !!(res && (res.success === true || (res.status && !res.error)));
  return { success, error: res?.error };
}

export async function logAccountChangeRequest(uid: string, currentAccount: string): Promise<{success: boolean; error?: string}> {
  // El script v4.5 no tiene acción de log genérico, usamos append a una hoja de soporte si existe
  const res = await sendToScript({
    action: 'append',
    tab: 'SOLICITUDES_SOPORTE',
    data: {
      UID_SOCIO: uid,
      TIPO: 'CAMBIO_CUENTA',
      DETALLES: `Solicitud de modificación de cuenta: ${currentAccount}`,
      FECHA: new Date().toISOString()
    }
  });
  return { success: !!(res && (res.success === true || !res.error)), error: res?.error };
...
export async function fetchShareholderAccount(uid: string): Promise<any | null> {
  // Con el script v4.5, obtenemos la tabla completa y filtramos (scoping en cliente)
  const data = await fetchTableData('DATOS_PAGO_SOCIOS');
  const record = data.find(r => norm(findValue(r, ['UID_SOCIO', 'uid'])) === norm(uid));
    ...
    const mapped: (Report & { _timestamp: number })[] = data
      ...
      .map(row => {
        ...
        return {
          ...
          visibleEnTodos: norm(findValue(row, ['VISIBLE_EN_TODOS', 'visible'])) === 'si' || String(findValue(row, ['VISIBLE_EN_TODOS', 'visible'])).toLowerCase() === 'true',
