...
const ShareholderProfile: React.FC<ShareholderProfileProps> = ({ user, onBack }) => {
  ...
  const handleSaveAccount = async () => {
    ...
    if (activeWithdrawalTab === 'crypto' ? !formData.walletAddress : (!formData.docNumber || !formData.accountNumber)) {
    ...
    try {
      ...
      if (res.success) {
        ...
        setSaveError(res.error || 'Error al procesar la solicitud en el servidor');
      }
    } catch (e) {
      console.error("Error guardando cuenta:", e);
      setSaveError('Fallo de conexión con el servidor maestro');
    } finally {
      ...
      {showConfirm && (
        <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-sm bg-white rounded-[40px] p-10 flex flex-col items-center text-center animate-in zoom-in-95">
            ...
            <p className="text-text-secondary text-xs font-medium leading-relaxed mb-4">Este canal será vinculado de forma permanente para la dispersión de utilidades. ¿Desea proceder?</p>
            
            {saveError && (
              <div className="w-full p-3 bg-red-50 border border-red-100 rounded-xl flex items-center gap-2 mb-6">
                <AlertOctagon size={16} className="text-red-500 shrink-0" />
                <p className="text-[10px] font-black text-red-600 uppercase text-left">{saveError}</p>
              </div>
            )}
